...
#include "zend_types.h" // for zval, zend_object, zend_resource
...
ZEND_API ZEND_ATTRIBUTE_PURE zend_long ZEND_FASTCALL zend_dval_to_lval_slow(double d);
...
static zend_always_inline ZEND_ATTRIBUTE_PURE zend_long zend_dval_to_lval_cap(double d, const zend_string *s)
{
	ZEND_IGNORE_VALUE(s);
	if (UNEXPECTED(!zend_finite(d))) {
		return 0;
