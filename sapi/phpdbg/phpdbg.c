...
# endif
	...
	php_ini_builder_prepend(&ini_builder, phpdbg_ini_hardcoded, sizeof(phpdbg_ini_hardcoded) - 1);
	...
	if (zend_extensions_len) {
		...
		while (zend_extension < zend_extensions_len) {
			...
			php_ini_builder_unquoted(&ini_builder, "zend_extension", sizeof("zend_extension") - 1, ze, ze_len);
